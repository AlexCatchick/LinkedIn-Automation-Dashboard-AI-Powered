...
class ApiService {
  ...
  async demoLogin(): Promise<AuthResponse> {
    const response: AxiosResponse<AuthResponse> = await this.api.post('/api/auth/demo-login', {});
    if (response.data.token) {
      localStorage.setItem('auth_token', response.data.token);
    }
    return response.data;
  ...
  async getCampaigns(): Promise<Campaign[]> {
    const response: AxiosResponse<ApiResponse<Campaign[]>> = await this.api.get('/api/campaigns');
    return response.data.data || [];
  }

  async getCampaign(id: string): Promise<Campaign> {
    const response: AxiosResponse<ApiResponse<Campaign>> = await this.api.get(`/api/campaigns/${id}`);
    return response.data.data!;
  }

  async createCampaign(data: { name: string; description?: string; intake_json: CampaignIntake }): Promise<Campaign> {
    const response: AxiosResponse<ApiResponse<Campaign>> = await this.api.post('/api/campaigns', data);
    return response.data.data!;
  }

  async updateCampaign(id: string, data: Partial<Campaign>): Promise<Campaign> {
    const response: AxiosResponse<ApiResponse<Campaign>> = await this.api.put(`/api/campaigns/${id}`, data);
    return response.data.data!;
  }

  async deleteCampaign(id: string): Promise<void> {
    await this.api.delete(`/api/campaigns/${id}`);
  }

  // Prospects
  async getProspects(campaignId?: string): Promise<Prospect[]> {
    const url = campaignId ? `/api/prospects?campaign_id=${campaignId}` : '/api/prospects';
    const response: AxiosResponse<ApiResponse<Prospect[]>> = await this.api.get(url);
    return response.data.data || [];
  }

  async getProspect(id: string): Promise<Prospect> {
    const response: AxiosResponse<ApiResponse<Prospect>> = await this.api.get(`/api/prospects/${id}`);
    return response.data.data!;
  }

  async createProspect(data: { campaign_id: string; linkedin_url: string; email?: string }): Promise<Prospect> {
    const response: AxiosResponse<ApiResponse<Prospect>> = await this.api.post('/api/prospects', data);
    return response.data.data!;
  }
  ...
  async updateProspect(id: string, data: Partial<Prospect>): Promise<Prospect> {
    const response: AxiosResponse<ApiResponse<Prospect>> = await this.api.put(`/api/prospects/${id}`, data);
    return response.data.data!;
  }

  async deleteProspect(id: string): Promise<void> {
    await this.api.delete(`/api/prospects/${id}`);
  }
  ...
  async getMessage(id: string): Promise<Message> {
    const response: AxiosResponse<ApiResponse<Message>> = await this.api.get(`/api/messages/${id}`);
    return response.data.data!;
  }
  ...
  async createMessage(data: {
    ...
    const response: AxiosResponse<ApiResponse<Message>> = await this.api.post('/api/messages', data);
    return response.data.data!;
  }
  ...
  async updateMessage(id: string, data: Partial<Message>): Promise<Message> {
    const response: AxiosResponse<ApiResponse<Message>> = await this.api.put(`/api/messages/${id}`, data);
    return response.data.data!;
  }
  ...
  async getSequences(campaignId?: string): Promise<Sequence[]> {
    const url = campaignId ? `/api/sequences?campaign_id=${campaignId}` : '/api/sequences';
    const response: AxiosResponse<ApiResponse<Sequence[]>> = await this.api.get(url);
    return response.data.data || [];
  }

  async getSequence(id: string): Promise<Sequence> {
    const response: AxiosResponse<ApiResponse<Sequence>> = await this.api.get(`/api/sequences/${id}`);
    return response.data.data!;
  }

  async createSequence(data: Partial<Sequence>): Promise<Sequence> {
    const response: AxiosResponse<ApiResponse<Sequence>> = await this.api.post('/api/sequences', data);
    return response.data.data!;
  }

  async updateSequence(id: string, data: Partial<Sequence>): Promise<Sequence> {
    const response: AxiosResponse<ApiResponse<Sequence>> = await this.api.put(`/api/sequences/${id}`, data);
    return response.data.data!;
  }

  async deleteSequence(id: string): Promise<void> {
    await this.api.delete(`/api/sequences/${id}`);
  }
  ...
  async getAnalytics(): Promise<Analytics> {
    const response: AxiosResponse<ApiResponse<Analytics>> = await this.api.get('/api/analytics/dashboard');
    return response.data.data!;
  }

  async getCampaignAnalytics(campaignId: string): Promise<Analytics> {
    const response: AxiosResponse<ApiResponse<Analytics>> = await this.api.get(`/api/analytics/campaigns/${campaignId}/performance`);
    return response.data.data!;
  }
